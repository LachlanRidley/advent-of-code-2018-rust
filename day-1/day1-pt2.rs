...
fn main() {
  ...
  let mut frequency = 0;
  let mut all_frequencies = HashSet::new();

  let mut iter = puzzle_items.iter().peekable();

  let repeated_frequency = loop {
    if iter.peek() == None {
      iter = puzzle_items.iter().peekable();
    }

    let puzzle_item = iter.next().unwrap();
    ...
    if all_frequencies.contains(&frequency) {
      break frequency;
    }

    all_frequencies.insert(frequency);
  };
